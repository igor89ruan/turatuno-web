import { NextRequest, NextResponse } from "next/server";
import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { z } from "zod";

export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) return NextResponse.json({ error: "Não autorizado." }, { status: 401 });

    const wu = await prisma.workspaceUser.findFirst({ where: { userId: session.user.id } });
    if (!wu) return NextResponse.json({ error: "Workspace não encontrado." }, { status: 404 });

    const body = await req.json();
    const schema = z.object({
      name:       z.string().min(1).max(80).optional(),
      limit:      z.number().positive().optional(),
      closingDay: z.number().min(1).max(31).optional(),
      dueDay:     z.number().min(1).max(31).optional(),
      color:      z.string().optional(),
      network:    z.enum(["visa","mastercard","elo","hipercard"]).nullable().optional(),
      lastFour:   z.string().length(4).nullable().optional(),
    });
    const data = schema.parse(body);

    const card = await prisma.creditCard.findFirst({
      where: { id: params.id, workspaceId: wu.workspaceId },
    });
    if (!card) return NextResponse.json({ error: "Cartão não encontrado." }, { status: 404 });

    const updated = await prisma.creditCard.update({ where: { id: params.id }, data });
    return NextResponse.json(updated);
  } catch (error) {
    console.error("[CARD_PATCH]", error);
    return NextResponse.json({ error: "Erro interno." }, { status: 500 });
  }
}

export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) return NextResponse.json({ error: "Não autorizado." }, { status: 401 });

    const wu = await prisma.workspaceUser.findFirst({ where: { userId: session.user.id } });
    if (!wu) return NextResponse.json({ error: "Workspace não encontrado." }, { status: 404 });

    const card = await prisma.creditCard.findFirst({
      where: { id: params.id, workspaceId: wu.workspaceId },
    });
    if (!card) return NextResponse.json({ error: "Cartão não encontrado." }, { status: 404 });

    await prisma.transaction.updateMany({ where: { creditCardId: params.id }, data: { creditCardId: null } });
    await prisma.creditCard.delete({ where: { id: params.id } });

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("[CARD_DELETE]", error);
    return NextResponse.json({ error: "Erro interno." }, { status: 500 });
  }
}
